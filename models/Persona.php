...
( select tc.generafiniquito from 
 contrato c 
inner join general.tipocontrato tc on tc.id=c.idtipocontrato  where c.eliminado=false  and c.idhistorialestadoempleado=hee.id order by c.id desc limit 1

) else(
 select tc.generafiniquito from 
 contrato c 
inner join general.tipocontrato tc on tc.id=c.idtipocontrato  where c.eliminado=false  and c.idhistorialestadoempleado=(select  he.id from general.historialestadoempleado he where he.eliminado=false and he.idempleado=hee.idempleado and he.id<hee.id order by he.id desc limit 1)  order by c.id desc limit 1
