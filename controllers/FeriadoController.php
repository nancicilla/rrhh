...
class FeriadoController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        if(isset($_POST['Feriado'])){
                ...
                if($model->fechafestividad!=''){
                if( Feriado::model()->find("t.fechafestividad::date='".$model->fechafestividad."'::date")==null ){    
                   if( $model->save()){
                   ...
                   else
                   {
                    ...
                    echo System::hasErrors('Ya existe un Feriado con esa fecha  ! ', $model);
                return;
                }
                
                }else{
                   echo System::hasErrors('La fecha es requerida  ! ', $model);
                return;  
                }
        }
    ...
    public function actionUpdate($id)
    {
        ...
        $model->fechafestividad=date ("d-m-Y",strtotime( $model->fechafestividad));
        if(isset($_POST['Feriado']))
        {
            $model->attributes=$_POST['Feriado'];
            if ($model->fechafestividad!=''){
            if( Feriado::model()->find("t.fechafestividad::date='".$model->fechafestividad."'::date and t.id<>".$model->id)==null && $model->fechafestividad!=''){
                if( $model->save()){
