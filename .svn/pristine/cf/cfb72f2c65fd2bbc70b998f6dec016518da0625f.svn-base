   ...
   /*
    ...
    public function actionCreate()
    {
        ...
        $fechaminima=  Yii::app()->rrhh
         ...
         $consulta='';
                 if (Yii::app()->user->checkAccess('action_rrhh_bono_admin')==1 && Yii::app()->user->checkAccess('action_rrhh_bono_adminBonoPlanillaTributaria')==1){
                    
                    
                 }else if(Yii::app()->user->checkAccess('action_rrhh_bono_adminBonoPlanillaTributaria')==1) {
                     $consulta=' and  t.id in(select id from general.bono where eliminado=false and esagrupador=true and enplanilla=false )';
                 }else{
                      $consulta=' and  t.id in(select id from general.bono where eliminado=false and esagrupador=true and enplanilla=true )';
    ...
    public function actionAdmin()
    {
      ...
      public function actionAdminBonoPlanillaTributaria()
    {
        Yii::app()->getClientScript()->scriptMap = array('jquery.js' => false, 'jquery.ui.js' => false, 'jquery-ui.min.js' => false);

        $model=new Bono('searchBonoPlanillaTributaria');
        $model->unsetAttributes();  // clear any default values
        ...
        $this->renderPartial('adminBonoPlanillaTributaria',array(
            'model'=>$model,
        ), false, true);
    }
    /**
