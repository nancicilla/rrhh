...
class OtrosgastosController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        if(isset($_POST['Otrosgastos'])){
                $model->attributes=$_POST['Otrosgastos'];
                if(Otrosgastos::model()->registrarOtrosGastosEmpleado ($_POST['Otrosgastos']['fecharegistro'], strtoupper( $_POST['Otrosgastos']['nombre']),$_POST['gridLista'])){                       
    ...
    public function actionAdmin()
    {
        ...
        if(isset($_GET['Otrosgastos'])){
                $model->attributes=$_GET['Otrosgastos'];
                if (!$model->validate()) {
	...
	{
		$model=Otrosgastos::model()->findByPk($id);
		if($model===null)
	 ...
	 * @param Otrosgastos $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='otrosgastos-form')
		{
        ...
        */
        ...
        if (!empty($_FILES['fd-file']) and is_uploaded_file($_FILES['fd-file']['tmp_name'])) {
            ...
            $nameSaveTemp = 'protected/modules/rrhh/tmp/OtrosGastosUpload' . rand(100, 999999) . '.' . $nameFile[sizeof($nameFile) - 1];
    ...
    /**
     * 
     * @param file $inputFileName,archivo con la informacion de empleados ,montos a asignar  a otros gastos
     ...
     * retorna un listado de empleado que contengan en su nombre completo a $_POST['nombrecompleto']
     */
