...
class Bonoespecial extends CActiveRecord
{
    ...
    public function rules()
    {
            ...
            return array(
                    array('asiento, estado,opcion', 'numerical', 'integerOnly'=>true),
                    array('nombre', 'length', 'max'=>100),
                    array('gestion, usuario', 'length', 'max'=>30),
                    array('monto', 'length', 'max'=>12),
                    array('porcentaje', 'length', 'max'=>15),
                    array('nombre,opcion, monto,fechadesde,fechahasta', 'required', 'on' => array('insert', 'update')),
                  
                    array('fechadesde, fechahasta,fechapago,  fecha, eliminado', 'safe'),
                    // The following rule is used by search().
                    // @todo Please remove those attributes that should not be searched.
                    array('id, nombre, gestion,opcion, fechadesde, fechahasta, asiento, monto, porcentaje, estado, usuario, fecha, eliminado', 'safe', 'on'=>'search'),
            );
    }
    ...
    /**
     * 
     * @param integer $idbono,  bono especial al cualrelacionaremos 
     * @param lista $lista, lista aportes/bonos
     * @param integer $estado,posibles valores son: 0 =aporte personal retirado,1=aporte personal activo, 2 = bonos ultima planilla ,3= aportes ultima planilla
     */
    ...
    public function guardarBeneficioAporte($bono, $lista, $estado) {
       
                  $cadena='';
                  for($i=1; $i<=count($lista);$i++){
                        if ($lista[$i]['estado'] == '1') {
                            $cadena = $cadena.'{"nombre":"'.$lista[$i]['nombre'].'","nombref":"'.$lista[$i]['nombref'].'"},';
