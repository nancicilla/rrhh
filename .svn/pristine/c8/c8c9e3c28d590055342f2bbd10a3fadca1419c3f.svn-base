...
class Planillaretroactivo extends CActiveRecord
{
    ...
    public function rules()
    {
            ...
            return array(
                    ...
                    array('mesinicio,porsistema, mesfin,monto,porcentaje', 'required', 'on' => array('insert', 'update')),
...
/**
     ...
     * @return boolean , true=Se va mostrar el boton Generar Asiento Incremento Indemnizacion 
     */
    public function MotrarGenerarAsientoAjusteIncremento($id)
    {
        $mostrar= Yii::app()->rrhh
            ->createCommand("select (not (p.asientoajusteincremento)  and  porsistema=true) from planillaretroactivo p where p.id=".SeguridadModule::dec($id))
            ->queryScalar();
            return $mostrar;
     ...
     * @param integer $id, id del Corte de Planilla
     */
     public function ConsolidarPlanillaIncrementoIndemnizacion($id) {
        $usuario = Yii::app()->user->getName();
        Yii::app()->contabilidad
            ->createCommand("select  public.generar_asiento_ajuste_incremento_indemnizacion($id,'$usuario') ")
            ->execute();
