...
Empleado.descargarReporte=function(ruta){
  ...
  if(ruta=='DescargarReporteIParaContrato'){
      vecb=[];
       var grillab = Empleado.getSGridView('tipo');
      for (var i = 1; i <= grillab.rows; i++) {
            vecb.push({  'estado': grillab.row(i).get('estado'),'id': grillab.row(i).get('id')});
        }
        var datos = this.prepareSend($('#' + this.groupForm).serialize()) + this.gestionSchemaMain();
           this.downloadFile(this.urlIni + this.url + '/'+ruta+'?' + datos+'& tipo='+JSON.stringify(vecb) );
